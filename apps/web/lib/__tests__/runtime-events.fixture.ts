...
test("normalises immutable run JSONL events with source timestamps", async () => {
  const files = {
    "/fixture/LegacyRuntime/goals/state/goal-a.json": JSON.stringify({ id: "goal-a", status: "completed", last_event_timestamp: "2026-08-04T09:30:00.000Z" }),
    "/fixture/LegacyRuntime/goals/state/queue-runner-status.json": "{}",
    "/fixture/LegacyRuntime/runs/goal-a/attempt-1-events.jsonl": [
...
test("malformed source rows fail soft as warnings", async () => {
  const runtime = await buildRuntimeSnapshot(roots, adapters({
    "/fixture/LegacyRuntime/goals/state/goal-bad.json": JSON.stringify({ id: "goal-bad", status: "running" }),
    "/fixture/LegacyRuntime/goals/state/queue-runner-status.json": "{}",
    "/fixture/LegacyRuntime/runs/goal-bad/attempt-1-events.jsonl": "{bad json",
  }));
...
test("event payloads redact synthetic secrets and private content fields", async () => {
  ...
  const files = {
    "/fixture/LegacyRuntime/goals/state/goal-secret.json": JSON.stringify({ id: "goal-secret", status: "running" }),
    "/fixture/LegacyRuntime/goals/state/queue-runner-status.json": "{}",
    "/fixture/LegacyRuntime/runs/goal-secret/attempt-1-events.jsonl": [
      JSON.stringify({
        type: "tool.completed",
...
test("event text redacts response output content, secrets, and filesystem paths", async () => {
  ...
  const files = {
    "/fixture/LegacyRuntime/goals/state/goal-sensitive-event.json": JSON.stringify({ id: "goal-sensitive-event", status: "running" }),
    "/fixture/LegacyRuntime/goals/state/queue-runner-status.json": "{}",
    "/fixture/LegacyRuntime/runs/goal-sensitive-event/attempt-1-events.jsonl": [
      JSON.stringify({
        type: "tool.completed",
        timestamp: "2026-08-04T09:20:00.000Z",
...
test("alert payloads redact synthetic secrets from source warnings", async () => {
  ...
  const runtime = await buildRuntimeSnapshot(roots, adapters({
    "/fixture/LegacyRuntime/goals/state/goal-alert.json": JSON.stringify({ id: "goal-alert", status: "running" }),
    "/fixture/LegacyRuntime/goals/state/queue-runner-status.json": "{}",
  }));
...
function commandAdapter(dirty: boolean): CommandAdapter {
  return {
    async execFile(file, args, options) {
      ...
      if (file === "git" && args[0] === "status") return { ok: true, stdout: dirty || options?.cwd === "/fixture/LegacyRuntime/dirty" ? " M file.ts\n" : "", stderr: "", code: 0 };
