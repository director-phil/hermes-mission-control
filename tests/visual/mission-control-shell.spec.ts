...
for (const viewport of [
  ...
  test(`Mission Control reference shell visual gate: ${viewport.name}`, async ({ page }) => {
    ...
    // For desktop (width > 1024), check desktop rail navigation
    if (viewport.width > 1024) {
      await expect(page.locator('.mc-desktop-rail')).toBeVisible();
      const overviewLink = page.locator('.mc-desktop-rail a[aria-current="page"]');
      await expect(overviewLink).toHaveText("Overview");
    }
    
    // Check the main heading and bounded evidence
    await expect(page.getByRole("heading", { name: "Overview" })).toBeVisible();
    // Overview now shows bounded truthful data (runtime evidence) instead of placeholder
    const hasControllerOrFleet = await page.getByText(/controllers|FLEET/).count();
    expect(hasControllerOrFleet).toBeGreaterThan(0);
    await expect(page.getByText(/target:/)).toBeVisible();
    // source: appears in environment banner and process evidence - verify at least one exists
    const hasSource = await page.locator('text=/source:/').count();
